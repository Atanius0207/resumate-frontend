        ...
        * {
        ...
        @media (max-width: 968px) {
            ...
            nav ul {
                ...
                background: var(--bg-nav); flex-direction: column; padding: 20px;
                box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1); gap: 20px;
                border-bottom: 1px solid var(--border-color);
            }
...
<body>
    ...
    <div class="modal-overlay" id="successModal">
        <div class="modal-content">
            ...
            <div>
                ...
                <a href="{{ route('test-editor', 1)}}" class="btn-modal btn-modal-secondary" onclick="editCV()">
    ...
    <script>
        ...
        function addEducation() {
            ...
            const html = `
                <div class="dynamic-item" data-index="${index}">
                    ...
                    ` : `
                        <div class="dynamic-item-header">
                            <h3 class="dynamic-item-title">Pendidikan #1</h3>
                        </div>
                    `}
                    <div class="form-row">
                        <div class="form-group">
        ...
        function removeEducation(index) {
            const item = document.querySelector(`#educationContainer .dynamic-item[data-index="${index}"]`);
            if (item) {
                item.remove();
            }
        }
        ...
        function toggleEducationEnd(index) {
            const checkbox = document.getElementById(`currentEducation${index}`);
            const endInput = document.getElementById(`educationEnd${index}`);
            
            if (checkbox.checked) {
                endInput.value = 'Sekarang';
        ...
        function addExperience() {
            ...
            const html = `
                <div class="dynamic-item" data-index="${index}">
                    ...
                    ` : `
                        <div class="dynamic-item-header">
                            <h3 class="dynamic-item-title">Pengalaman #1</h3>
                        </div>
                    `}
                    <div class="form-row">
                        <div class="form-group">
        ...
        function removeExperience(index) {
            const item = document.querySelector(`#experienceContainer .dynamic-item[data-index="${index}"]`);
            if (item) {
                item.remove();
            }
        }
        ...
        function toggleExperienceEnd(index) {
            const checkbox = document.getElementById(`currentWork${index}`);
            const endInput = document.getElementById(`experienceEnd${index}`);
        ...
        function addSkillTag(skill, type) {
            ...
            const input = container.querySelector('.skill-input-wrapper');
            
            const tag = document.createElement('span');
            tag.className = 'skill-tag';
            tag.innerHTML = `
                ${skill}
                <i class="fas fa-times" onclick="removeSkillTag(this, '${skill}', '${type}')"></i>
            `;
        ...
        function removeSkillTag(element, skill, type) {
            element.parentElement.remove();
            
            if (type === 'technical') {
                const index = technicalSkills.indexOf(skill);
                if (index > -1) technicalSkills.splice(index, 1);
            } else {
                const index = softSkills.indexOf(skill);
                if (index > -1) softSkills.splice(index, 1);
            }
        }
