...
<style>
    /* =========================================
       1. THEME VARIABLES (Sama persis dengan Create/Index)
       ========================================= */
    :root {
        /* --- Light Mode Default --- */
    ...
    .create-template-wrapper { display: grid; grid-template-columns: 2fr 1fr; gap: 24px; align-items: start; }
    .card-section { background: var(--bg-card); border: 1px solid var(--border-color); border-radius: 12px; padding: 24px; margin-bottom: 24px; box-shadow: var(--shadow-sm); }
    .section-title { font-size: 16px; font-weight: 700; color: var(--text-main); margin-bottom: 20px; padding-bottom: 12px; border-bottom: 1px solid var(--border-color); }
    ...
    .b-input, .b-select { padding: 8px 10px; border: 1px solid var(--border-color); border-radius: 4px; font-size: 13px; width: 100%; background: var(--bg-input); color: var(--text-main); }
    .b-label { font-size: 10px; color: var(--text-muted); display: block; margin-bottom: 2px; }
    .icon-grip { color: var(--text-light); cursor: move; }

    /* Buttons */
    .btn-sm-icon { width: 32px; height: 32px; display: flex; align-items: center; justify-content: center; border-radius: 6px; border: 1px solid var(--border-color); cursor: pointer; background: var(--bg-card); color: var(--text-muted); transition: 0.2s; }
    .btn-sm-icon:hover { background: var(--primary-hover-bg); color: var(--primary-color); border-color: var(--primary-color); }
    
    .btn-add { background: var(--btn-soft-success-bg); color: var(--btn-soft-success-text); border: 1px solid var(--btn-soft-success-bg); padding: 10px 16px; border-radius: 6px; cursor: pointer; font-size: 13px; font-weight: 600; display: inline-flex; align-items: center; gap: 5px; }
    ...
    .btn-submit { background: var(--primary-color); color: white; border: none; padding: 12px 30px; border-radius: 8px; font-weight: 600; cursor: pointer; transition: 0.2s; box-shadow: 0 4px 6px rgba(0,0,0,0.1); }
    .btn-submit:hover { opacity: 0.9; transform: translateY(-1px); }

    /* Upload & Toggle */
    .upload-area { border: 2px dashed var(--border-color); border-radius: 12px; padding: 20px; text-align: center; cursor: pointer; background: var(--bg-body); transition: 0.2s; position: relative; }
    .upload-area:hover { border-color: var(--primary-color); background: var(--primary-hover-bg); }
    #image-preview { width: 100%; border-radius: 8px; margin-bottom: 10px; object-fit: cover; }
    ...
    @media (max-width: 900px) {
        ...
        .builder-row-grid { grid-template-columns: 1fr 1fr; grid-template-rows: auto auto auto; gap: 8px 12px; }
...
<div style="margin-bottom: 24px; display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap;">
    <div>
        <h1 style="font-size: 24px; font-weight: 700; color: var(--text-main);">Edit Template</h1>
        <p style="color: var(--text-muted);">Perbarui informasi dan struktur template <strong>{{ $template->name }}</strong>.</p>
    </div>
...
<form id="editForm" action="{{ route('admin.templates.update', $template->id) }}" method="POST" enctype="multipart/form-data">
    @csrf 
    @method('PUT')
            ...
            <div class="card-section">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="name" class="form-input @error('name') is-invalid @enderror" 
                           placeholder="Contoh: Modern Blue Professional" value="{{ old('name', $template->name) }}" required>
                ...
                <div class="form-group">
                    ...
                    <textarea name="description" class="form-textarea" rows="4"
                              placeholder="Jelaskan keunggulan template ini...">{{ old('description', $template->description) }}</textarea>
                </div>

                <div class="form-group">
                    <label class="form-label">Tags (Pisahkan koma)</label>
                    <input type="text" name="tags" class="form-input" 
                           placeholder="Minimalis, ATS, Biru, Creative" 
                           value="{{ old('tags', is_array($template->tags) ? implode(',', $template->tags) : $template->tags) }}">
                </div>
            </div>
                ...
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(140px, 1fr)); gap: 20px;">
                    <div class="form-group">
                        ...
                        <select name="type" id="typeSelect" class="form-select @error('type') is-invalid @enderror">
                            <option value="free" {{ old('type', $template->type) == 'free' ? 'selected' : '' }}>Gratis (Free)</option>
                            <option value="pro" {{ old('type', $template->type) == 'pro' ? 'selected' : '' }}>Berbayar (Pro)</option>
                        </select>
                    ...
                    <div class="form-group" id="priceField" style="display:none;">
                        ...
                        <input type="number" name="price" class="form-input @error('price') is-invalid @enderror" 
                               placeholder="0" value="{{ old('price', $template->price) }}">
        ...
        <div class="right-col">
            <div class="card-section">
                ...
                <div class="upload-area @error('thumbnail') is-invalid @enderror" onclick="document.getElementById('fileInput').click()">
                    <img id="image-preview" src="{{ $template->thumbnail ? Storage::url($template->thumbnail) : '#' }}" 
                         alt="Preview" style="{{ $template->thumbnail ? 'display:block' : 'display:none' }}">
                    
                    <div id="placeholder-content" style="{{ $template->thumbnail ? 'display:none' : 'display:block' }}">
                        <i class="fas fa-cloud-upload-alt" style="font-size:32px; color:var(--text-muted); margin-bottom:10px;"></i>
                        <p style="font-size:13px; font-weight:500; color:var(--text-main);">Klik untuk Ganti</p>
                        <p style="font-size:11px; color:var(--text-muted);">Max 2MB (JPG/PNG)</p>
                    </div>
                    <input type="file" name="thumbnail" id="fileInput" hidden accept="image/*" onchange="previewImage(this)">
                </div>
            ...
            <div class="card-section">
                ...
                <div class="form-group">
                    ...
                    <select name="category" class="form-select @error('category') is-invalid @enderror">
                        @php $currentCat = is_array($template->category) ? ($template->category[0] ?? '') : $template->category; @endphp
                        <option value="" disabled>Pilih Kategori</option>
                        <option value="Professional" {{ old('category', $currentCat) == 'Professional' ? 'selected' : '' }}>Professional</option>
                        <option value="Creative" {{ old('category', $currentCat) == 'Creative' ? 'selected' : '' }}>Creative</option>
                        <option value="Simple" {{ old('category', $currentCat) == 'Simple' ? 'selected' : '' }}>Simple</option>
                        <option value="Akademik" {{ old('category', $currentCat) == 'Akademik' ? 'selected' : '' }}>Akademik</option>
                    </select>
                ...
                <div style="display:flex; justify-content:space-between; margin-bottom:15px; align-items:center;">
                    ...
                    <label class="switch"><input type="checkbox" name="is_active" value="1" {{ old('is_active', $template->is_active) ? 'checked' : '' }}><span class="slider"></span></label>
                </div>
                ...
                <div style="display:flex; justify-content:space-between; align-items:center;">
                    ...
                    <label class="switch"><input type="checkbox" name="is_new" value="1" {{ old('is_new', $template->is_new) ? 'checked' : '' }}><span class="slider"></span></label>
                </div>
            </div>
        </div>
...
<script>
    ...
    togglePrice(); // Run immediately on load

    // --- 3. LOGIC FORM BUILDER (EDIT MODE) ---
    
    // LOAD EXISTING DATA FROM DATABASE
    let schemaData = @json($template->form_schema ?? []); 
    
    // If schema is empty/null for some reason, use default empty structure to avoid crash
